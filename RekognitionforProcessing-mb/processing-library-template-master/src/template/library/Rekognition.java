package rekognitionupdate.faces;
...
public class Rekognition {
	...
	public RekognitionUpdate[] detectFacesPath(String path) {
	...
	public RekognitionUpdate[] detectFacesURL(String url) {
		...
		post.addData("jobs","face_part_gender_emotion_age_glass_beauty_race");
	...
	public RekognitionUpdate[] recognize(String s) {
	...
	public RekognitionUpdate[] detect(String s) {
		Pattern p = Pattern.compile("^http",Pattern.CASE_INSENSITIVE);
		Matcher m = p.matcher(s);
		if (m.find()) {
	...
	public RekognitionUpdate[] detectFaces(File f) {
		PostRequest post = new PostRequest(api);
		post.addData("api_key", rekog_key);
		post.addData("api_secret", rekog_secret);
	...
	public RekognitionUpdate[]  recognizeFacesPath(String path) {
		PostRequest post = new PostRequest(api);
		post.addData("api_key", rekog_key);
		post.addData("api_secret", rekog_secret);
		...
		post.addData("job_list", "face_recognize_part_gender_emotion_age_glass_beauty_race");
		File f = new File(p5.sketchPath(path));
	...
	public RekognitionUpdate[]  recognizeFacesURL(String url) {
		PostRequest post = new PostRequest(api);
		post.addData("api_key", rekog_key);
		post.addData("api_secret", rekog_secret);
	...
	public RekognitionUpdate[] facesFromJSON(String content) {
		JSONObject data = p5.parseJSONObject(content);
		JSONArray facearray = data.getJSONArray("face_detection");

		RekognitionUpdate[] faces = new RekognitionUpdate[facearray.size()];
		for (int i = 0; i < faces.length; i++) {
			faces[i] = new RekognitionUpdate();  // Fix to include width and height!
